...
export function businessDestination(candidate: string | null) {
  return candidate && (["/business", "/business/dashboard", "/business/candidates", "/business/attendance", "/business/requirements", "/business/onboarding", "/business/company", "/business/account"].includes(candidate)
    || /^\/business\/requirements\/[a-zA-Z0-9_-]{1,64}(?:\/(?:edit|candidates|attendance))?$/.test(candidate) || /^\/business\/(?:candidates|attendance)\/[a-zA-Z0-9_-]{1,64}$/.test(candidate))
    ? candidate : "/business";
}
