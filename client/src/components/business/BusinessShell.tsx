...
import { ArrowUpRight, Building2, CircleHelp, LayoutDashboard, LogOut, Menu, ShieldCheck, UserRound } from "lucide-react";
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";
import { useBusiness } from "./BusinessProvider";
import { BusinessWordmark } from "./BusinessUI";

const links = [{ href: "/business/dashboard", label: "Dashboard", icon: LayoutDashboard }, { href: "/business/company", label: "Company profile", icon: Building2 }, { href: "/business/account", label: "Account & security", icon: UserRound }];
...
export function BusinessShell({ children }: { children: ReactNode }) {
  ...
  const navigation = <nav className="zb-biz-nav" aria-label="Business workspace">{links.map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={pathname === href ? "page" : undefined} onClick={() => setOpen(false)}><Icon aria-hidden="true" />{label}</Link>)}</nav>;
  ...
  return <div className="zb-biz-shell">
    ...
    <div className="zb-biz-workarea">
      <header className="zb-biz-topbar">
        ...
        <span className="zb-biz-topbar-title">{pathname === "/business/onboarding" ? "Company setup" : pathname.startsWith("/business/requirements/") ? "Requirement brief" : links.find(link => link.href === pathname)?.label ?? "Business workspace"}</span>
