...
export function CandidateProfile({ id, accountId, admin = false, onChanged, compact = false }: { id: string; accountId: string; admin?: boolean; onChanged?: () => void; compact?: boolean }) {
  ...
  return <div className="zb-cand-profile">
    ...
    <section className="zb-cand-section"><h2><UserRound aria-hidden="true" />Profile at a glance</h2><p className="zb-cand-preserve">{item.summary}</p><dl className="zb-cand-facts"><div><dt>Experience</dt><dd>{item.experience || "To be discussed"}</dd></div><div><dt>Available from</dt><dd>{item.availableFrom ? businessDate(item.availableFrom) : "To be confirmed"}</dd></div></dl>
      ...
      {item.hasPrivateResume ? <PrivateResume className="zb-biz-button zb-biz-button--secondary" path={`${admin ? "/admin/candidate-management" : "/business/candidates"}/${id}/resume`} /> : cv ? <a className="zb-biz-button zb-biz-button--secondary" href={cv} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer"><FileText aria-hidden="true" />Open CV<ExternalLink aria-hidden="true" /></a> : <p className="zb-cand-cv-note"><FileText aria-hidden="true" />CV not available. Ask the account team to provide it.</p>}
...
function HistoryEvent({ event }: { event: CandidateEvent }) {
  ...
  return <li><span><Icon aria-hidden="true" /></span><div><h3>{title}</h3><time dateTime={event.createdAt}>{businessDate(event.createdAt, true)} IST · {event.actorRole === "BUSINESS" ? "Business team" : "ZOBHUNGER team"}</time><p className="zb-cand-preserve">{event.note}</p>{event.workerMessage && <p className="zb-cand-preserve"><strong>Shared with worker: </strong>{event.workerMessage}</p>}{event.interviewAt && <div className="zb-cand-interview"><strong>{businessDate(event.interviewAt, true)} IST</strong><span>{event.interviewMode?.replaceAll("_", " ")}</span><p className="zb-cand-preserve">{event.interviewDetails}</p><small>Requested time; confirmation requires coordination with the account team.</small></div>}</div></li>;
}
...
function ReviewForm({ item, onSaved }: { item: BusinessCandidate; onSaved: (notice: string) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try { await reviewCandidate(item.id, { ...payload, workerMessage: String(form.get("workerMessage") || "") }); onSaved(action === "INTERVIEW" ? "Interview request recorded for the account team to coordinate." : "Your review has been saved."); }
  ...
  return <section className="zb-cand-section zb-cand-review"><h2><ListReviewIcon />Your next step</h2><form onSubmit={submit}><fieldset disabled={busy}>
    ...
    <p className="zb-cand-muted">Visible to your business and the ZOBHUNGER operations team.</p><label className="zb-cand-field"><span>Message shared with worker (optional)</span><textarea name="workerMessage" rows={3} maxLength={1500} placeholder="Next steps or interview instructions the applicant should see." /></label><p className="zb-cand-muted">Worker portal applicants can read this message and the interview time in My applications. Internal feedback stays private.</p>
