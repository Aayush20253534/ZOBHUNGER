...
export function CandidateProfile({ id, accountId, admin = false, onChanged, compact = false }: { id: string; accountId: string; admin?: boolean; onChanged?: () => void; compact?: boolean }) {
  ...
  return <div className="zb-cand-profile">
    ...
    {item.status === "SELECTED" && !item.revokedAt && <Link className="zb-biz-text-link" href={admin ? "/admin/attendance?tab=assignments" : `/business/requirements/${item.requirementId}/attendance`}><CalendarClock aria-hidden="true" />{admin ? "Set up or review team assignments" : "View assigned team attendance"}<ExternalLink aria-hidden="true" /></Link>}
