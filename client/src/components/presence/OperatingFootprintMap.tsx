        ...
        <g className="zb-footprint-routes" aria-hidden="true">
          {markers
            ...
            .map((marker) => (
              <path
                key={marker.name}
                d={`M ${headquarters.x} ${headquarters.y} Q ${(headquarters.x + marker.x) / 2 + 8} ${(headquarters.y + marker.y) / 2 - 8} ${marker.x} ${marker.y}`}
              />
            ))}
        </g>
