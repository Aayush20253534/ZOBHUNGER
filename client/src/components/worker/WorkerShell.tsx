...
import { BriefcaseBusiness, CalendarCheck2, FileCheck2, ArrowUpRight, Bookmark, CircleHelp, LayoutDashboard, LogOut, Menu, RefreshCw, Search, ShieldCheck, UserRound, WalletCards, WifiOff } from "lucide-react";
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";
import { useWorker } from "./WorkerProvider";
import { WorkerAlert, WorkerWordmark } from "./WorkerUI";

const links = [{ href: "/worker", label: "Dashboard", icon: LayoutDashboard, exact: true }, { href: "/worker/applications", label: "My applications", icon: FileCheck2 }, { href: "/worker/assignments", label: "My assignments", icon: BriefcaseBusiness }, { href: "/worker/attendance", label: "My attendance", icon: CalendarCheck2 }, { href: "/worker/earnings", label: "Earnings", icon: WalletCards }, { href: "/worker/jobs", label: "Find work", icon: Search }, { href: "/worker/saved-jobs", label: "Saved jobs", icon: Bookmark }, { href: "/worker/profile", label: "My profile & CV", icon: UserRound }];
export function WorkerShell({ children }: { children: ReactNode }) {
  ...
  const navigation = <nav className="zw-nav" aria-label="Worker navigation">{links.map(({ href, label, icon: Icon, exact }) => <Link href={href} key={href} aria-current={exact ? pathname === href ? "page" : undefined : pathname === href || pathname.startsWith(`${href}/`) ? "page" : undefined} onClick={() => setOpen(false)}><Icon aria-hidden="true" />{label}</Link>)}</nav>;
  ...
  return <div className="zw-shell"><a className="zw-skip" href="#worker-content">Skip to worker content</a>
    ...
    <div className="zw-workarea"><header className="zw-topbar"><div className="zw-mobile-menu"><Sheet open={open} onOpenChange={setOpen}><SheetTrigger className="zw-menu-trigger" aria-label="Open worker navigation"><Menu aria-hidden="true" /></SheetTrigger><SheetContent side="left" className="zw zw-drawer"><SheetHeader><SheetTitle>Your worker space</SheetTitle><SheetDescription>Find roles, save opportunities and manage your profile.</SheetDescription></SheetHeader>{account}{navigation}{bottom}<WorkerAlert message={error} /></SheetContent></Sheet></div><span>{links.find(link => link.exact ? pathname === link.href : pathname === link.href || pathname.startsWith(`${link.href}/`))?.label || "Worker space"}</span><span className="zw-verified"><ShieldCheck aria-hidden="true" />Email verified</span><Link href="/for-workers" aria-label="Back to the website">Website<ArrowUpRight aria-hidden="true" /></Link></header>
