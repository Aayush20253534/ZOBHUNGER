...
import { getWorkerWorkspace, loginWorker, requestWorkerEmail, resetWorkerPassword, verifyWorkerEmail } from "@/services/worker.service";
import { WorkerAlert, WorkerWordmark, workerError } from "./WorkerUI";

type Mode = "login" | "forgot" | "verify" | "reset";
const content = {
  login: { eyebrow: "APPROVED WORKER ACCESS", title: "Your worker space is ready when you are.", copy: "Sign in with the access issued to you after profile review and approval." },
...
export function WorkerAccess({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(""); setMessage("");
    if (mode === "reset" && (password !== confirmation || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password))) {
  ...
  const needsEmail = mode === "login" || mode === "forgot" || (mode === "verify" && !token);
  ...
  const buttonText = mode === "login" ? "Sign in" : mode === "verify" && token ? "Verify my email" : mode === "reset" ? "Save new password" : "Send email link";
  return <div className={`zw-auth zw-auth--${mode}`}><header className="zw-auth-top"><WorkerWordmark /><Link href="/jobs">Browse public jobs<ArrowUpRight aria-hidden="true" /></Link></header>
    <main className="zw-auth-grid"><aside className="zw-auth-story"><div className="zw-auth-photo"><Image src="/images/careers/field-opportunities-960.webp" alt="ZOBHUNGER field executives working together in a market" width={960} height={640} sizes="(max-width: 800px) 100vw, 46vw" priority /><div className="zw-photo-label"><ShieldCheck aria-hidden="true" /><span>A place for your skills.<strong>A path to your next opportunity.</strong></span></div></div><div className="zw-auth-story-copy"><p className="zw-eyebrow">YOUR SKILLS, OUT IN THE WORLD</p><h2>Good work begins with you.</h2><div className="zw-access-steps">{[{ icon: FileUser, title: "Submit", copy: "Share your profile and CV first" }, { icon: ShieldCheck, title: "Review", copy: "Our team verifies the required details" }, { icon: Bookmark, title: "Continue", copy: "Approved workers receive the next steps" }].map(({ icon: Icon, title, copy }, index) => <div key={title}><span><Icon aria-hidden="true" /></span><small>0{index + 1}</small><h3>{title}</h3><p>{copy}</p></div>)}</div></div></aside>
      <section className="zw-card zw-auth-card"><span className="zw-icon">{mode === "verify" || mode === "forgot" ? <Mail aria-hidden="true" /> : <KeyRound aria-hidden="true" />}</span><p className="zw-eyebrow">{content[mode].eyebrow}</p><h1>{content[mode].title}</h1><p className="zw-muted">{content[mode].copy}</p>
        {mode === "verify" && search.get("delivery") === "unavailable" && !completed && <WorkerAlert message="Your worker access was issued, but the verification email could not be sent. Try again below or contact our team." />}
        ...
        {completed ? <Link className="zw-button" href={workerAccessHref("login", next)}>{mode === "reset" ? "Sign in with new password" : "Sign in to your worker space"}<ArrowRight aria-hidden="true" /></Link> : readingLink ? <p role="status">Reading your link…</p> : mode === "reset" && !token ? <Link className="zw-button" href={workerAccessHref("forgot-password", next)}>Request a new reset link<Mail aria-hidden="true" /></Link> : <form className="zw-auth-form" onSubmit={submit}>
          ...
          {mode === "reset" && <><ul className="zw-password-rules" aria-label="Password requirements">{rules.map(rule => <li className={rule.done ? "is-done" : ""} key={rule.label}><Check aria-hidden="true" />{rule.label}</li>)}</ul><PasswordField label="Confirm password *" value={confirmation} onChange={setConfirmation} confirm fresh /></>}
