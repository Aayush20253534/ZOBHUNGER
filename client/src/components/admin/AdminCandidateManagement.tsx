...
function ApplicationPicker({ accountId, onSelect, requirementId }: { requirementId?: string; accountId: string; onSelect: (item: ApplicationOption) => void }) {
  return <OptionPicker<ApplicationOption> requirementId={requirementId} accountId={accountId} kind="applications" placeholder="Applicant name, city or job title" onSelect={onSelect} label={item => item.name} description={item => `${item.job.title} · ${item.city || "Location to confirm"}${item.job.requirementId ? " · Linked requirement" : ""}`} />;
}
