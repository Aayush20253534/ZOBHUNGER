...
export function AdminCandidateManagement() {
  ...
  return <div className="zb-biz zb-cand zb-cand-admin"><Link className="zb-biz-text-link" href="/admin"><ArrowLeft aria-hidden="true" />Operations dashboard</Link>{loading ? <CandidateLoading /> : error ? <CandidateError error={error} retry={() => setAuthVersion(value => value + 1)} admin /> : data?.user.role !== "ADMIN" ? <CandidateError error={new ApiError("Use your administrator account to manage candidate sharing.", 403)} retry={() => setAuthVersion(value => value + 1)} admin /> : <>
    ...
    <div id="candidate-review-desk"><CandidateWorkspace key={data.user.id} admin accountId={data.user.id} refreshToken={version} /></div>
  </>}</div>;
}
...
function CandidateShareForm({ accountId, onShared }: { accountId: string; onShared: () => void }) {
  ...
  return <section className="zb-cand-share"><div className="zb-cand-section-title"><Send aria-hidden="true" /><div><h2>Share a candidate</h2><p>Choose the receiving business and the application you have reviewed.</p></div></div>
    ...
    <div><h3><span>02</span> Reviewed application</h3>{application ? <div className="zb-cand-selection"><strong>{application.name}</strong><p>{application.job.title} · {application.city || "Location to confirm"}</p><p className="zb-cand-preserve">{application.experience || "Experience to discuss"}</p>{cv && <a href={cv} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer"><FileText aria-hidden="true" />Review submitted CV</a>}<button type="button" disabled={busy} onClick={() => setApplication(null)}><X aria-hidden="true" />Change application</button></div> : <ApplicationPicker accountId={accountId} onSelect={item => { setApplication(item); setSkills(item.skills.join(", ")); }} />}</div></div>
    ...
    <p className="zb-cand-muted">Only open requirements attached to an active business account are listed. Demo jobs and rejected applications are excluded.</p>
  </section>;
}
...
function ApplicationPicker({ accountId, onSelect }: { accountId: string; onSelect: (item: ApplicationOption) => void }) {
  return <OptionPicker<ApplicationOption> accountId={accountId} kind="applications" placeholder="Applicant name, city or job title" onSelect={onSelect} label={item => item.name} description={item => `${item.job.title} · ${item.city || "Location to confirm"}`} />;
}
function OptionPicker<T extends { id: string }>({ accountId, kind, placeholder, label, description, onSelect }: { accountId: string; kind: "requirements" | "applications"; placeholder: string; label: (item: T) => string; description: (item: T) => string; onSelect: (item: T) => void }) {
  ...
  const request = useCallback((signal: AbortSignal) => getCandidateOptions<T>(kind, query, page, signal), [kind, query, page]);
  const { data, error, loading } = useBusinessResource(`${accountId}:${kind}:${query}:${page}:${version}`, request);
  return <div className="zb-cand-picker"><form onSubmit={event => { event.preventDefault(); setQuery(input.trim()); setPage(1); }}><label className="zb-cand-search"><Search aria-hidden="true" /><span className="zb-cand-sr-only">Search {kind}</span><input value={input} onChange={event => setInput(event.target.value)} placeholder={placeholder} maxLength={100} /></label><button className="zb-cand-icon-button" aria-label={`Search ${kind}`}><ArrowRight aria-hidden="true" /></button></form>
    {loading ? <p role="status">Loading {kind}…</p> : error ? <CandidateError error={error} retry={() => setVersion(value => value + 1)} admin /> : data && <><ul>{data.items.map(item => <li key={item.id}><button type="button" onClick={() => onSelect(item)}><span><strong>{label(item)}</strong><small>{description(item)}</small></span><ArrowRight aria-hidden="true" /></button></li>)}</ul>{!data.total && <p>No eligible {kind} found. {query ? "Try a different search." : kind === "requirements" ? "The business must submit an owned requirement first." : "Applicants must apply to a real job opening first."}</p>}<CandidatePagination page={data.page} totalPages={data.totalPages} onChange={setPage} label={`${kind} pages`} /></>}
  </div>;
}
