...
export function EmployeeComplianceLanding() {
  ...
  return <div className="zhr-shell zcomp-shell">
    ...
    {!profile ? <main className="zcomp-access-wrap"><section className="zcomp-access-card">
      <div className="zcomp-access-intro">
        <div className="zcomp-access-icon"><ShieldCheck /></div>
        <p className="zhr-kicker">SECURE EMPLOYEE ACCESS</p>
        <h1>Open your compliance record.</h1>
        <p className="zcomp-access-copy">Use the same details submitted in your Employee Joining record. No PF, ESIC or government-portal password is required.</p>
        <div className="zcomp-access-points">
          <span><LockKeyhole />Private verification</span>
          <span><FileCheck2 />Linked to joining data</span>
        </div>
      </div>
      <div className="zcomp-access-panel">
        <div className="zcomp-access-panel-head"><div><span>Identity verification</span><strong>Confirm your employee record</strong></div><small><i /> Secure session</small></div>
        {error && <div className="zhr-alert zhr-alert--error"><strong>Unable to continue.</strong><span>{error}</span></div>}
        <form className="zcomp-access-form" onSubmit={verify}>
          <label className="zhr-field"><span>Employee ID *</span><input required maxLength={80} autoComplete="off" placeholder="ZBH-..." value={identity.employeeNumber} onChange={e => setIdentity(v => ({ ...v, employeeNumber: e.target.value.toUpperCase() }))} /></label>
          <label className="zhr-field"><span>Registered email *</span><input required type="email" autoComplete="email" placeholder="name@example.com" value={identity.personalEmail} onChange={e => setIdentity(v => ({ ...v, personalEmail: e.target.value }))} /></label>
          <label className="zhr-field"><span>Date of birth *</span><input required type="date" autoComplete="bday" value={identity.dateOfBirth} onChange={e => setIdentity(v => ({ ...v, dateOfBirth: e.target.value }))} /></label>
          <label className="zhr-field"><span>Last 4 digits of Aadhaar *</span><input required inputMode="numeric" autoComplete="off" pattern="[0-9]{4}" maxLength={4} placeholder="••••" value={identity.aadhaarLast4} onChange={e => setIdentity(v => ({ ...v, aadhaarLast4: e.target.value.replace(/\D/g, "").slice(0,4) }))} /></label>
          <div className="zcomp-access-action"><span>Details must match your approved Employee Joining record.</span><button className="zhr-button zhr-button--submit" type="submit" disabled={busy}>{busy ? <LoaderCircle className="zhr-spin" /> : <BadgeCheck />}{busy ? "Verifying…" : "Verify & continue"}</button></div>
        </form>
      </div>
    </section></main> : <main className="zcomp-workspace">
