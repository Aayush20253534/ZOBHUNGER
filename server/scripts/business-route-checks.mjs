...
export async function checkBusinessRoutes(baseUrl) {
  ...
  for (const check of checks) {
    ...
    if (check.path === "/health" && (body.success !== true || body.data?.features?.businessPortal !== true || body.data?.features?.businessDashboard !== true)) {
      throw new Error("This API does not report the business dashboard. Deploy the latest backend commit and check NEXT_PUBLIC_API_URL.");
    }
