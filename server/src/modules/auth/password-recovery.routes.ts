...
import { requestAdminRecovery, requestBusinessRecovery, resetAdminPassword, resetBusinessPassword } from "./password-recovery.service.js";
...
export const adminPasswordRecoveryRouter = Router();
adminPasswordRecoveryRouter.post("/forgot-password", recoveryLimiter, validate({ body: requestRecoverySchema }), (_req, res) => {
  if (!recoveryEmailConfigured()) throw new HttpError(503, "Account recovery is temporarily unavailable. Please contact ZOBHUNGER support.", { code: "RECOVERY_UNAVAILABLE" });
  const requestId = res.locals.requestId;
  void requestAdminRecovery(res.locals.validated.body.email, requestId).catch(() => logger.warn("admin.recovery_failed", { requestId }));
  res.status(202).json(apiSuccessResponse("If an active administrator account uses this email, a recovery link will be sent. Check your inbox and spam folder.", { accepted: true }));
});
adminPasswordRecoveryRouter.post("/reset-password", authRateLimiter, validate({ body: resetPasswordSchema }), async (_req, res) => {
  const { token, password } = res.locals.validated.body;
  const result = await resetAdminPassword(token, password);
  res.json(apiSuccessResponse("Administrator password updated. Sign in with your new password.", result));
});
