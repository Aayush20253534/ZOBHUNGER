...
export const listPlacementCandidatesController: RequestHandler = async (_req, res) => {
  const result = await getPlacementCandidates(userId(res), res.locals.validated.query as PlacementCandidateQuery);
  res.json(apiSuccessResponse("Candidates retrieved", {
