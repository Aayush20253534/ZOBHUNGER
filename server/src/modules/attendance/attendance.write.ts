...
async function eligibleCandidate(tx: Prisma.TransactionClient, id: string) {
  const row = await tx.businessCandidate.findUnique({ where: { id }, select: {
    id: true, requirementId: true, name: true, jobTitle: true, status: true, revokedAt: true,
  ...
  if (!row || row.revokedAt || row.status !== "SELECTED" || row.requirement.status === "CLOSED" || owner?.role !== "BUSINESS" || !owner.isActive) {
...
export async function createAssignment(userId: string, input: CreateAssignment) {
  return prisma.$transaction(async tx => {
    const reference = await tx.businessCandidate.findUnique({ where: { id: input.candidateId }, select: { requirementId: true } });
...
export async function updateAssignment(userId: string, id: string, input: UpdateAssignment) {
  return prisma.$transaction(async tx => {
    ...
    if (await tx.attendanceRecord.count({ where: { assignmentId: id } }) || await tx.attendanceCorrection.count({ where: { assignmentId: id } })) {
...
export async function endAssignment(userId: string, id: string, input: { revision: number; endDate: string; note: string }) {
  return prisma.$transaction(async tx => {
    ...
    if (await tx.attendanceRecord.count({ where: later }) || await tx.attendanceCorrection.count({ where: later })) throw new HttpError(409, "The end date cannot exclude recorded attendance or correction requests.", { code: "ASSIGNMENT_HAS_HISTORY" });
...
export async function cancelAssignment(userId: string, id: string, input: { revision: number; note: string }) {
  return prisma.$transaction(async tx => {
    ...
    if (await tx.attendanceRecord.count({ where: { assignmentId: id } }) || await tx.attendanceCorrection.count({ where: { assignmentId: id } })) throw new HttpError(409, "This assignment has attendance history. End it instead of cancelling it.", { code: "ASSIGNMENT_HAS_HISTORY" });
...
async function saveRecord(tx: Prisma.TransactionClient, assignment: Awaited<ReturnType<typeof findAssignment>>, date: string, input: AttendanceValues, userId: string, source: string) {
