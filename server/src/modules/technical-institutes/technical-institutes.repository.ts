import { PlacementCellApplicationStatus, Prisma } from "../../generated/prisma/client.js";
...
export function findTechnicalInstituteApplicationForAdmin(id: string) {
  return prisma.technicalInstituteApplication.findUnique({ where: { id } });
}
...
export async function reviewTechnicalInstituteApplicationWithAudit(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!statusChanged && !notesChanged) return { kind: "unchanged" as const, entity: current };
    ...
    await tx.auditLog.create({
      data: {
        ...
        action: statusChanged ? "TECHNICAL_INSTITUTE_APPLICATION_STATUS_CHANGED" : "TECHNICAL_INSTITUTE_APPLICATION_REVIEW_NOTES_UPDATED",
