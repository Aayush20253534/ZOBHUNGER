...
Object.assign(process.env, { DATABASE_URL: process.env.TEST_DATABASE_URL, NODE_ENV: 'test', REDIS_ENABLED: 'false', JWT_SECRET: 'workflow-test-only-not-a-production-secret', LOG_LEVEL: 'error', API_LOG_FORMAT: 'combined', CORS_ORIGIN: 'http://localhost:3000' });
...
test('P3.4 applications and P3.5 attendance connect trusted workers to operations', async t => {
  try {
    ...
    async function account(role = 'WORKER', verified = true) { const user = await prisma.user.create({ data: { email: `${prefix}-${++serial}@example.test`, passwordHash: 'unused-test-password-hash-for-integration-tests-only', emailVerified: verified ? new Date() : null } }); users.push(user); return user; }
    ...
    const profile = await prisma.workerProfile.create({ data: { userId: worker.id, fullName: 'Asha Executive', phone: `98765432${String(serial).padStart(2, '0')}`, city: 'Delhi', state: 'Delhi', skills: ['Retail', 'Customer service'], education: [], workExperience: [], revision: 0 } });
    await prisma.workerProfile.create({ data: { userId: other.id, fullName: 'Other Executive', phone: `98765433${String(serial).padStart(2, '0')}`, city: 'Delhi', state: 'Delhi', education: [], workExperience: [] } });
    ...
    async function job() { const row = await prisma.job.create({ data: { slug: `${prefix}-${++serial}`, title: 'Market Executive', location: 'Delhi', city: 'Delhi', category: 'Sales', engagementType: 'FULL_TIME', description: 'Join our team', status: 'OPEN' } }); jobs.push(row.id); return row; }
    ...
    const req = await prisma.workforceRequirement.create({ data: { businessProfileId: bp.id, submittedByUserId: business.id, companyName: 'Test Retail', contactPerson: 'Test Lead', businessEmail: 'test@retail.test', roles: [], positions: 1, status: 'ACTIVE' } }); requirements.push(req.id);
    ...
    const setup = { location: 'Delhi Central', supervisor: 'Field supervisor', startDate: addDays(today, -20), endDate: addDays(today, 20), shiftStart: '09:00', shiftEnd: '18:00', graceMinutes: 10, workingDays: ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'] };
    ...
    await t.test('operations and business reviews publish safe updates and protect submitted CVs', async () => {
      ...
      ok(await request(`/business/candidates/${candidateId}/reviews`, { user: business, method: 'POST', body: { action: 'INTERVIEW', revision: 0, note: 'PRIVATE business feedback', interviewAt: new Date(addDays(today, 3)) } }));
      const detail = ok(await request(`/workers/applications/${applicationId}`, { user: worker })); assert.equal(detail.application.stage, 'INTERVIEW_REQUESTED'); assert.match(JSON.stringify(detail), /PRIVATE business feedback/);
    ...
    await t.test('assignment schedule and attendance are isolated by worker ID, with bounded IST dates', async () => {
      ...
      const month = ok(await request(`/workers/assignments/${assignmentId}/calendar?month=2024-02`, { user: worker })); assert.equal(month.days.length, 29); assert.ok(month.days.every(item => item.workingDay === true));
    ...
    try {
      ...
      await prisma.attendanceCorrection.deleteMany({ where: { assignment: own } });
      await prisma.attendanceRecord.deleteMany({ where: { assignment: own } });
      await prisma.workforceAssignment.deleteMany(own);
      ...
      // Delete profile and profile-related records
      await prisma.workerResume.deleteMany({ where: { profile: { userId: { in: users.map(u => u.id) } } } });
      await prisma.workerProfile.deleteMany({ where: { userId: { in: users.map(u => u.id) } } });
      await prisma.businessProfile.deleteMany({ where: { userId: { in: users.map(u => u.id) } } });
