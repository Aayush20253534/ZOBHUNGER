...
function response(path) {
  ...
  if (path === "/api/backend/health/ready") return Response.json({ status: "ready", service: "zobhunger-api", revision });
  if (path === "/api/backend/health") return Response.json({ success: true, data: { status: "ok", service: "zobhunger-api", revision } });
...
test("HTML error pages and missing backend modules cannot pass as a healthy API", async () => {
  for (const bad of [new Response("Gateway failure", { status: 502 }), new Response("<html>404</html>", { headers: { "Content-Type": "text/html" } }), Response.json({ success: true, data: { service: "wrong-api", status: "ok" } })]) {
